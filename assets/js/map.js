...
const baseMaps = {
    "Game Map": L.tileLayer('https://motortown-bbt.com/assets/tiles/{z}_{x}_{y}.avif', {
        minZoom: 1,
        maxZoom: 6,
        tileSize: 256,
        noWrap: true,
...
async function updatePlayerPositions() {
    try {
        ...
        const response = await fetch('https://motortown-bbt.com/api/playerlocations');
...
async function fetchAndUpdateNPCs() {
    try {
        if (DEBUG_MODE) currentDateTime = getCurrentDateTime();
        const response = await fetch('https://motortown-bbt.com/api/npcs');
        const data = await response.json();
...
async function fetchAndUpdateGarages() {
    try {
        const response = await fetch('https://motortown-bbt.com/api/garages');
        const data = await response.json();
